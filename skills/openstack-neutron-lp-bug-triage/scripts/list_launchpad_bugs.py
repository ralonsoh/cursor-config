...
def fetch_bugs(project, start_date, end_date, status=None, enrich=False):
...
def main(argv=None):
    ...
    parser.add_argument(
        '--status',
        help='Optional Launchpad status filter (for example: New)',
    )
    parser.add_argument(
    ...
    try:
        bugs = fetch_bugs(
            ...
            status=args.status, enrich=args.enrich)
