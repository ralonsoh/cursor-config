---
name: openstack-neutron-lp-bug-triage
description: >-
  Summarize Neutron Launchpad bugs filed in a date range, grouped by
  importance, with assignee and active patch status. Use when triaging
  Neutron bugs, preparing bug deputy reports, or when the user asks for a
  Launchpad bug summary for a period of time.
---

# OpenStack Neutron Launchpad Bug Triage

Produce a triage summary of Neutron bugs created in an inclusive date range.
...
```bash
python3 scripts/list_launchpad_bugs.py \
  --start YYYY-MM-DD \
  --end YYYY-MM-DD \
  --json \
  --enrich
...
Run the command from this skill directory
(`openstack-neutron-lp-bug-triage/` under the configured skills path).
...
```bash
python3 scripts/list_launchpad_bugs.py --start 2026-05-26 --end 2026-05-27 --json --enrich
```
