---
name: openstack-testing-patches
description: >-
  Create temporary testing patches for OpenStack CI: duplicate Zuul jobs,
  duplicate test classes/methods, modify tox regex to target specific tests,
  or repeat a single test multiple times. Use when the user asks to create a
  testing patch, stress-test a CI job, run a test multiple times, or debug
  flaky tests in Zuul CI.
---

# OpenStack Testing Patches
